...
// Get nutrition goals (don't create automatically)
export async function getNutritionGoals(userId: string): Promise<NutritionGoals | null> {
  try {
    ...
    if (goals) {
      console.log('Found nutrition goals:', goals.id)
      return goals
    }

    console.log('No nutrition goals found for user:', userId)
    return null
  } catch (error) {
...
export async function createOrUpdateNutritionGoals(userId: string, goals: CreateNutritionGoalsInput): Promise<NutritionGoals> {
  try {
    console.log('Creating or updating nutrition goals for user:', userId)
    
    const now = new Date().toISOString()

    // First check if goals already exist for this user
    ...
    if (fetchError && fetchError.code !== 'PGRST116') {
      ...
      throw new Error(`Failed to check existing goals: ${fetchError.message}`)
    }

    if (existingGoals) {
      // Update existing goals
      console.log('Updating existing nutrition goals for user:', userId)
      ...
      if (updateError) {
        ...
        throw new Error(`Failed to update nutrition goals: ${updateError.message}`)
      }

      console.log('Updated nutrition goals:', updatedGoals.id)
      return updatedGoals
    } else {
      // Create new goals
      console.log('Creating new nutrition goals for user:', userId)
      ...
      if (insertError) {
        console.error('Error creating nutrition goals:', insertError)
        throw new Error(`Failed to create nutrition goals: ${insertError.message}`)
      }

      console.log('Created nutrition goals:', newGoals.id)
      return newGoals
    }
  } catch (error) {
    console.error('Error in createOrUpdateNutritionGoals:', error)
    throw error
  }
}

// Create nutrition goals (legacy function - keeping for backward compatibility)
export async function createNutritionGoals(userId: string, goals: CreateNutritionGoalsInput): Promise<NutritionGoals> {
  try {
    console.log('Creating nutrition goals for user:', userId)
    ...
    if (error) {
      ...
      throw new Error(`Failed to create nutrition goals: ${error.message}`)
    }

    console.log('Created nutrition goals:', newGoals.id)
    return newGoals
  } catch (error) {
    console.error('Error in createNutritionGoals:', error)
    throw error
  }
}

// Get or create nutrition goals (legacy function - keeping for backward compatibility)
export async function getOrCreateNutritionGoals(userId: string): Promise<NutritionGoals> {
  try {
    ...
    return await createNutritionGoals(userId, defaultGoals)
  } catch (error) {
